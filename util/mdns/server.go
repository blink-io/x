...
func (s *Server) recv(c *net.UDPConn) {
	...
	for {
		...
		if err := s.parsePacket(buf[:n], from); err != nil {
			slog.Error("[ERR] mdns: Failed to handle query", slog.Any("error", err))
		}
	}
}
...
func (s *Server) parsePacket(packet []byte, from net.Addr) error {
	...
	if err := msg.Unpack(packet); err != nil {
		slog.Error("[ERR] mdns: Failed to unpack packet", slog.Any("error", err))
		return err
	}
...
func (s *Server) probe() {
	...
	for i := 0; i < 3; i++ {
		if err := s.SendMulticast(q); err != nil {
			slog.Error("[ERR] mdns: failed to send probe", slog.Any("error", err.Error()))
		}
	...
	for i := 0; i < 3; i++ {
		if err := s.SendMulticast(resp); err != nil {
			slog.Error("[ERR] mdns: failed to send announcement", slog.Any("error", err.Error()))
		}
		select {
		case <-timer.C:
			timeout *= 2
